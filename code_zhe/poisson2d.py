...
class GPRLatent:
    ...
    #u_test:  M1 x M2
    def __init__(self, bvals, X_col, src_vals, jitter, X_test, u_test):
...
def test_2d():
    ...
    u = lambda x,y: jnp.sin(5.7*jnp.pi*x)*jnp.cos(95.7*jnp.pi*x) +jnp.sin(7.23*jnp.pi*y)*jnp.cos(67.2*jnp.pi*y)
    #test points
    M = 300
    ...
    model_PIGP = GPRLatent(bvals, X_col, src_vals,  1e-6, X_test, u_test)
    np.random.seed(123)
    random.seed(123)
    #nepoch = 250000
    nepoch = 50000
