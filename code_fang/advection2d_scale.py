...
class GPRLatent:
    ...
    def __init__(self, bvals, X_col, src_vals, jitter, X_test, u_test, trick_paras =None, fix_dict=None):
        ...
        self.beta_scale = trick_paras['beta_scale']
    ...
    def loss(self, params, key):
        ...
        eq_ll = 0.5 * self.Nc * log_v - 0.5 * jnp.exp(log_v) * jnp.sum(jnp.square(  self.beta_scale*self.beta*U_x  + U_y - self.src_vals ))
    ...
    def train(self, nepoch):
        ...
        params = {
            ...
            "kernel_paras_1": {'log-w': np.log(1/Q)*np.ones(Q), 'log-ls': np.zeros(Q), 'freq': np.linspace(0, 1, Q)*1, 'log-w-matern': np.zeros(1),'log-ls-matern': np.zeros(1)},
        ...
        print('gen fig ...')
        extra_name = '/beta_scale_%.3f'%( self.beta_scale)
        utils.make_fig_2d(self, params, log_dict, extra_name)
...
# scale * \beta u_x + u_y=  (1-scale) * \beta u_x

#u should be the target function
def get_source_val(u, x_pos, y_pos,beta,beta_scale):
    x_mesh, y_mesh = np.meshgrid(x_pos, y_pos, indexing='ij')
    x_vec = x_mesh.reshape(-1)
    y_vec = y_mesh.reshape(-1)
    return beta_scale*beta*(vmap(grad(u, 0), (0, 0))(x_vec, y_vec)) + vmap(grad(u, 1), (0, 0))(x_vec, y_vec)
...
def test_2d(trick_paras,fix_dict=
    ...
    beta_scale = trick_paras['beta_scale']
    equation_dict = {
        'Advection-Eq-scale':lambda x,y:  jnp.sin(x-beta*y),
    }
    ...
    src_vals = get_source_val(u, x_pos_tr, y_pos_tr,beta,beta_scale)
