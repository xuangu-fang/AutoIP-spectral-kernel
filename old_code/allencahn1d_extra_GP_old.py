...
class GPRLatent:
    ...
    def step(self, params, opt_state, key):
        ...
        # '''scale by schedule'''
        # updates = optax.scale_by_schedule(updates,
        #                                   self.schedule(opt_state.step))
    ...
    def train(self, nepoch):
        ...
        for i in tqdm.tqdm(range(nepoch)):
            ...
            if i % (nepoch / 20) == 0:
                ...
                matern_w_list.append(
                    np.exp(params['kernel_paras']['log-w-matern']))

                matern_ls_list.append(
                    np.exp(params['kernel_paras']['log-ls-matern']))
            ...
            if i == int(nepoch / 2):
                # frezzes the kernel parameters as the current value
                fix_dict = {
                    'log-w': 1,
                    'freq': 1,
                    'log-ls': 1,
                ...
                fix_kernel_paras = {
                    'log-w': jnp.copy(params['kernel_paras']['log-w']),
                    'log-ls': jnp.copy(params['kernel_paras']['log-ls']),
                    'freq': jnp.copy(params['kernel_paras']['freq'] * 1),
                    'log-w-matern': np.zeros(1),
                    'log-ls-matern': np.zeros(1),
                }

                params['kernel_paras']['log-w-matern'] = np.zeros(1)
                params['kernel_paras']['log-ls-matern'] = np.zeros(1)

                # self.cov_func.update_fix_set(fix_dict, fix_kernel_paras)
                self.cov_func = trick_paras['kernel'](fix_dict,
                                                      fix_kernel_paras)
        ...
        other_paras = self.trick_paras['other_paras']
        utils.make_fig_1d_extra_GP(self, params, log_dict, other_paras)
...
if __name__ == '__main__':
    ...
    trick_list = [
        ...
        #     'lr': 1e-2, 'llk_weight': 100.0, 'kernel': kernels_new.Matern52_Cos_1d, 'nepoch': 100000, 'freq_scale': 100, 'logdet': True},
        {
            ...
            'lr': 1e-2,
            'llk_weight': 100.0,
            'kernel': kernels_new.Matern52_Cos_add_Matern_1d,
            'kernel_extra': kernels_new.Matern52_Cos_1d,
            'nepoch': 100000,
            'freq_scale': 100,
            'logdet': True,
            'other_paras': '-extra-GP'
